...
**方法**: GET  
**路径**: /v1/finance/balance-sheet  
**描述**: 分页查询资产负债表，按 `end_date` 倒序返回。
...
| message | string | 状态信息 |
| data.total | int | 总条数 |
| data.db_source | string | 数据源，通常为 `balance_sheet` |
...
| data.items[].total_assets | number | 资产总计 |
| data.items[].total_liab | number | 负债合计 |
| data.items[].total_hldr_eqy_exc_min_int | number | 股东权益合计（不含少数股东权益） |
    ...
    "items": [
      {
        ...
        "end_date": "2025-12-31",
        "report_type": "1",
        "total_assets": 5800000000000.0,
        "total_liab": 5300000000000.0,
        "total_hldr_eqy_exc_min_int": 500000000000.0,
        "update_flag": "1"
      }
    ]
  }
