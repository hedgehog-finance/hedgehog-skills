...
**功能**: 完成刺猬投研AI接入安装配置。
**适用场景**: 用户首次接入刺猬投研AI、重新绑定账号或要求重新完成接入安装配置时。
**工作流**:
1. 从用户对话中提取 `token` 和 `accountId`，若缺少则主动询问
2. 调用 `Tool-1` 执行接入安装配置
3. 把工作空间`hedgehog-workspace`中的AGENTS.md文件里的`Group Chats`章节内容改为
    You are prohibited from participating in group chats.
4. 脚本执行完毕后，告知用户：
   > 已配置成功，即将重启 Gateway 使其生效。重启过程中连接会短暂断开，属正常现象。
5. 重启 Gateway 使配置生效。
6. 执行Tool-7，并返回JSON格式结果。
...
**适用场景**: 当用户询问刺猬投研AI配置是否成功、接入是否完成、上次配置是否正常时。
**工作流**:
1. 调用 `Tool-6` 查询本地配置状态记录
2. 若状态显示已配置成功，直接告知用户“刺猬投研AI集成龙虾成功”
3. 若未找到状态记录，告知用户未找到配置结果，建议重新执行接入安装配置
