...
description: >
  技术指标计算 Skill：覆盖 SMA、EMA、RSI、MACD、BOLL（布林带）、OBV、KDJ、ATR、VWAP 9 个常用指标，
  每个指标提供两种调用方式——
  ① 自带数据计算（调用方传行情数组）；
  ② 服务端数据计算（按股票代码与日期区间，由服务端从 ciweiai-data 拉行情后计算）。
  触发词：均线/SMA/EMA、RSI、MACD、布林带/BOLL、能量潮/OBV、KDJ、ATR、VWAP；
  计算指标、技术指标、前复权指标。
  不适用：行情数据查询（用 hedgehog-company-index-data）。
version: 1.0.0
---
...
- **请求/响应**：均为 `application/json`；输入字段原样透传，不做业务转换。
- **参数小数化**：百分比/比例字段统一传小数（如 `0.05`=5%）。
- **`params` 覆盖规则**：仅传需覆盖的键；未知键返回 400。
- **错误约定**：
  - `400 {"error": "..."}` — 请求体校验失败或计算异常（指标名错误、缺必需字段、未知 params 键等）。
  - `200 {"error": "..."}` — 部分接口对无效业务输入返回错误对象，调用方需在业务层判断。
...
| 字段 | 类型 | 是否必需 | 含义 |
|------|------|----------|------|
| `date` | string\|null | 建议提供 | 行情日期或时间，建议 `YYYY-MM-DD`。若所有行日期可解析，服务端按日期升序排序。 |
...
| `end_date` | string(YYYY-MM-DD) | 是 | — | 结束日期，不得早于 `start_date` |
| `indicator` | string | 是 | — | 指标名（每个 Tool 固定）|
| `params` | object | 否 | `{}` | 指标参数覆盖，仅允许该指标 `default_params` 中已有键 |
...
| `limit` | integer | 否 | `1000` | 走 `/stock/daily` 时的条数限制，范围 1~1000；前复权接口不使用 |

### 通用返回字段

两种调用方式返回结构基本一致；服务端数据计算额外多 `source` 字段。

| 字段 | 类型 | 含义 |
|------|------|------|
| `indicator` | string | 规范化后的指标名 |
| `category` | string | 指标类别（`overlap`/`momentum`/`volatility`/`volume`/`trend`/...）|
| `params` | object | 实际计算参数 = 默认参数 + 调用方覆盖项 |
| `output_columns` | string[] | 本次输出列名 |
| `data[]` | object[] | 逐行结果，每行含 `date`（或 `index`）及所有输出列 |
| `latest` | object\|null | 从后向前找到的首条所有输出列均非空记录；数据量不足时可能为 `null` |
...
> 提示：MACD 需至少 `slow + signal` 根以上 K 线才会出现非空值，建议传入 60 根以上数据。
...
- **判读**：股价触及上/下轨可能反转；带宽 `BBB` 收窄后扩张常伴随突破。
...
> 注意：OBV 需 `volume`，**必须**使用 `price_adjustment:"none"`（前复权接口不返回成交量）。
...
- **指标名**：`vwap`
- **类别**：overlap
- **默认参数**：`{ "anchor": "D" }`（`anchor` 控制重置周期，`D`=按日，`W`=按周，`M`=按月）
- **必需字段**：`high`、`low`、`close`、`volume`
- **输出列**：`VWAP_{anchor}`（如 `VWAP_D`）
- **判读**：股价位于 VWAP 上方多头占优；常用于日内或日频比较。
...
> 注意：VWAP 需 `volume`，**必须**使用 `price_adjustment:"none"`（前复权接口不返回成交量）。
...
脚本在 HTTP 非 2xx 时以非 0 退出码退出，并把响应体打印到 stderr。

---

## 调用建议

1. **数据量保证**：数据长度建议 ≥ `params.length × 3`，避免大量 `null` 行；MACD 等多周期指标建议 ≥ 60 根。
2. **复权选择**：
   - 价格类指标（SMA/EMA/RSI/MACD/BOLL/KDJ/ATR）推荐 `price_adjustment:"forward"` 消除分红/送股造成的价格跳变；
   - 成交量类指标（OBV/VWAP）必须用 `price_adjustment:"none"`。
3. **取最新值**：一般业务只关心 `latest`；如需做形态判断或绘图请取整段 `data`。
