# KDJ

**方法**: POST  
**路径**: /v1/indicators/calculate 或 /v1/indicators/calculate-from-data  
**描述**: 计算K、D、J三线随机指标。脚本会固定提交 `"indicator": "kdj"`。

## 请求

### Headers
| 字段 | 值 |
|------|----|
| Content-Type | application/json |
| X-API-Token | 从环境变量 `CIWEI_AI_TOKEN` 读取 |

### data 模式 Body 参数
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| data | object[] | 是 | K线数据数组；每行必须包含 `high`、`low` 和 `close` |
| data[].date | string | 否 | 行情日期或时间；建议 `YYYY-MM-DD` |
| data[].high | number | 是 | 最高价 |
| data[].low | number | 是 | 最低价 |
| data[].close | number | 是 | 收盘价 |
| period | number | 否 | 计算周期，默认 `9`；脚本映射为后端 `params.length` |
| signal | number | 否 | 信号平滑周期，默认 `3` |
| params | object | 否 | 可传 `{ "period": 9, "signal": 3 }` 或 `{ "length": 9, "signal": 3 }` 覆盖参数 |

### 行情查询模式 Body 参数
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| stock_code | string | 是 | 股票代码，例如 `000001.SZ`；不传 `data` 时脚本调用 `/v1/indicators/calculate-from-data` |
| start_date | string | 是 | 开始日期，格式 `YYYY-MM-DD` |
| end_date | string | 是 | 结束日期，格式 `YYYY-MM-DD` |
| price_adjustment | string | 否 | 价格复权方式，默认 `none`，可传 `forward` |
| limit | number | 否 | 查询行情条数限制，默认由服务端决定 |
| period | number | 否 | 计算周期，默认 `9`；脚本映射为后端 `params.length` |
| signal | number | 否 | 信号平滑周期，默认 `3` |
| params | object | 否 | 可传 `{ "period": 9, "signal": 3 }` 或 `{ "length": 9, "signal": 3 }` 覆盖参数 |

### 请求示例
```json
{
  "data": [
    { "date": "2026-05-18", "high": 12.30, "low": 12.00, "close": 12.10 },
    { "date": "2026-05-19", "high": 12.50, "low": 12.20, "close": 12.35 },
    { "date": "2026-05-20", "high": 12.60, "low": 12.30, "close": 12.42 }
  ],
  "period": 9,
  "signal": 3
}
```

### 行情查询请求示例
```json
{
  "stock_code": "000001.SZ",
  "start_date": "2026-05-01",
  "end_date": "2026-05-22",
  "period": 9,
  "signal": 3,
  "price_adjustment": "forward"
}
```

## 响应

### 成功 (200)
| 字段 | 类型 | 说明 |
|------|------|------|
| indicator | string | 固定为 `kdj` |
| category | string | 指标类别 |
| params | object | 实际用于计算的参数 |
| output_columns | string[] | 输出列名，如 `K_9_3`, `D_9_3`, `J_9_3` |
| data | object[] | 逐行计算结果 |
| latest | object\|null | 最近一条完整结果 |
| source | object | 行情查询模式返回的数据来源信息 |

### 响应示例
```json
{
  "indicator": "kdj",
  "category": "momentum",
  "params": { "length": 9, "signal": 3 },
  "output_columns": ["K_9_3", "D_9_3", "J_9_3"],
  "data": [{ "date": "2026-05-20", "K_9_3": null, "D_9_3": null, "J_9_3": null }],
  "latest": null
}
```

### 错误码
| 状态码 | 含义 |
|--------|------|
| 400 | 参数缺失或格式错误 |
| 500 | 服务端异常 |
