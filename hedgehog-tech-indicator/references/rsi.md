...
**描述**: 计算相对强弱指数。脚本会固定提交 `"indicator": "rsi"`。
...
| period | number | 否 | RSI周期，默认 `14`；脚本映射为后端 `params.length` |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| indicator | string | 固定为 `rsi` |
| category | string | 指标类别 |
| params | object | 实际用于计算的参数 |
| output_columns | string[] | 输出列名，如 `RSI_14` |
| data | object[] | 逐行计算结果 |
...
```json
{
  ...
  "data": [{ "date": "2026-05-20", "RSI_14": null }],
  "latest": null
}
```
