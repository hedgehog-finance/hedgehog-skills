...
**描述**: 计算成交量加权平均价。脚本会固定提交 `"indicator": "vwap"`。
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| data | object[] | 是 | K线数据数组；每行必须包含 `high`、`low`、`close` 和 `volume` |
...
| params | object | 否 | VWAP无额外指标参数，默认 `{}` |
...
{
  "data": [
    { "date": "2026-05-18", "high": 12.30, "low": 12.00, "close": 12.10, "volume": 1200000 },
    { "date": "2026-05-19", "high": 12.50, "low": 12.20, "close": 12.35, "volume": 1350000 },
    { "date": "2026-05-20", "high": 12.60, "low": 12.30, "close": 12.42, "volume": 1280000 }
  ]
}
```
...
| 字段 | 类型 | 说明 |
|------|------|------|
| indicator | string | 固定为 `vwap` |
| category | string | 指标类别 |
| params | object | 实际用于计算的参数 |
| output_columns | string[] | 输出列名，如 `VWAP_D` |
| data | object[] | 逐行计算结果 |
...
```json
{
  ...
  "data": [{ "date": "2026-05-20", "VWAP_D": 12.44 }],
  "latest": { "date": "2026-05-20", "VWAP_D": 12.44 }
}
```
