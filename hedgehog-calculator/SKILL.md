...
description: >
  一个常用计算器：包含金融计算（终值/现值/折扣/加价/年金/贷款/投资回报）、
...
## 全局约定
- 所有百分比、利率、收益率、折扣率字段统一使用**小数形式**：`0.05` 表示 5%，`0.2` 表示 20%。
- 调用方式：`node scripts/call-api.js <method> '<params-json>'`。
- 返回内容由各接口决定，可能是对象、数字或带 `error` 字段的业务错误对象。

---

### Tool-1: 终值计算

**功能**：根据现值、年利率、年限和复利频次，计算未来价值。

**适用场景**：投资/储蓄按指定复利频率增长后的终值估算（需要原始浮点精度）。
...
**返回字段说明**：直接返回未来价值（number），不做四舍五入。

---

### Tool-2: 现值计算

**功能**：根据未来金额、年贴现率、年限和复利频次，计算当前价值。

**适用场景**：现金流折现、估算未来金额在当前时点的价值。
...
| `rate` | number | 是 | — | 年贴现率小数，`0.05` 表示 5% |
...
**返回字段说明**：直接返回折现后的当前价值（number）。

---

### Tool-3: 折扣金额计算

**功能**：根据商品原价和折扣比例，计算折扣金额、成交价和节省比例。
...
| `discount_percent` | number | 是 | 折扣比例小数，`0.2` 表示优惠 20% |
...
|------|------|------|------|
| `cost` | number | 是 | 成本金额 |
| `markup_percent` | number | 是 | 加价比例小数，`0.3` 表示成本上浮 30% |
...
**功能**：计算每期期末等额付款，在每期利率下累计到最后一期的价值。
...
| `rate` | number | 是 | 每期利率小数，`0.005` 表示每期 0.5% |
...
**返回字段说明**：直接返回普通年金终值（number）；若 `rate` 为 0，则返回 `payment × periods`。

---

### Tool-6: 普通年金现值计算

**功能**：将未来每期等额付款按每期利率折现到当前的价值。

**适用场景**：贷款额度估算、定投本金价值评估、租赁付款现值。
...
**返回字段说明**：直接返回普通年金现值（number）；若 `rate` 为 0，则返回 `payment × periods`。
...
**适用场景**：房贷、车贷、消费贷月供测算（年利率为 0 时按本金平均分摊到每月）。
...
| `annual_rate` | number | 是 | 贷款年利率小数，`0.042` 表示 4.2% |
...
| `total_payment` | number | 还款总额（保留 2 位小数） |
...
**功能**：根据初始本金、最终金额和持有年限，计算总收益率与复合年化收益率。

**适用场景**：基金/股票/项目持有期间的回报率评估。

**输入参数**：

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `principal` | number | 是 | 初始本金（≤ 0 时返回业务错误对象） |
| `final_value` | number | 是 | 投资期末最终金额 |
| `years` | number | 是 | 投资年限（≤ 0 时返回业务错误对象） |
...
**功能**：对一组数字计算数量、均值、（总体）方差、标准差、中位数、众数、最小值、最大值和极差。

**适用场景**：批量数据的描述性统计；注意方差按总体方差公式（除以 `n`）。
...
| `mode` | array \| string | 众数列表；全部值出现次数相同时返回字符串 `无众数` |
| `min` | number | 最小值 |
| `max` | number | 最大值 |
| `range` | number | 极差（保留 4 位小数） |
...
**功能**：将一个数值从原始单位换算到目标单位，支持长度、重量、面积、温度。

**适用场景**：日常单位转换；调用方需保证 `from_unit`/`to_unit` 属于同一类别（接口不强制校验）。

**输入参数**：

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `value` | number | 是 | 待换算数值 |
| `from_unit` | string | 是 | 原始单位（大小写会归一化） |
...
| `result` | number | 换算后的数值（普通单位 6 位小数，温度 2 位小数） |
...
**功能**：根据出生日期和参考日期计算年龄拆分（年/月/日）、总天数和估算总月份。
...
**功能**：计算两个日期之间的绝对间隔，返回天数、周数（按 7 天）、月数（按 30 天）和年数（按 365.25 天）。
...
**适用场景**：基础代数求解；当 `a = 0` 时会返回 `无解` 或 `无穷多解`。
...
**功能**：求解形如 `ax² + bx + c = 0` 的一元二次方程，根据判别式返回两个不等实根、重根或两个共轭复根；`a = 0` 时退化为一次方程结果。
