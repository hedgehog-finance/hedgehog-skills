...
description: >
  常用计算器：包含数学表达式计算（四则/指数/取模/三角函数/常用数学函数）、
  金融计算（终值/现值/折扣/加价/年金/贷款/投资回报）、统计分析、
  单位换算、日期计算（年龄/日期差）、一元一次/二次方程求解。
  触发词：计算终值；计算现值；计算折扣；计算加价；年金计算；贷款月供；
  投资回报；统计分析；单位换算；计算年龄；计算日期差；一元一次方程；
  二次方程；数学表达式；四则运算；三角函数；开平方；对数。
  不适用：多元方程。
version: 1.0.0
---
...
**功能**：计算纯字符串数学表达式，支持四则运算、指数、取模、常用数学函数和角度制三角函数。

**适用场景**：基础算式、复合表达式、开方、对数、三角函数和常量 `pi`/`e` 计算。

**输入参数**：

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `expression` | string | 是 | 待计算的数学表达式，最长 300 字符 |

**执行方法**：
```
node scripts/call-api.js calculate '{"expression":"sin30 + sqrt(16)"}'
```
...
| 字段 | 类型 | 含义 |
|------|------|------|
| `result` | number | 计算结果，最多保留 10 位小数 |
| `parsed` | string | 预处理后的表达式 |
| `elapsed_ms` | number | 服务端计算耗时，单位毫秒 |

**支持示例**：

| 类别 | 示例 |
|------|------|
| 基础四则 | `1+2*3` → 7；`10//3` → 3；`10%3` → 1；`2^3` → 8 |
| 三角函数 | `sin30` → 0.5；`cos(60)` → 0.5；`tan45` → 1.0。三角函数统一使用角度制 |
| 数学函数 | `sqrt(16)` → 4.0；`log(100)` → 2.0；`ln(e)` → 1.0；`abs(-5)` → 5.0 |
| 内置常量 | `pi`、`e`，例如 `2*pi*5` |
...
| 状态码 | 场景 | 响应格式 |
|------|------|------|
| `422` | 表达式语法非法、含非法字符、不支持的函数、除零、结果为 `inf` 或 `nan` | `{"detail":"..."}` |
| `408` | 计算超过 2 秒 | `{"detail":"计算超时"}` |
