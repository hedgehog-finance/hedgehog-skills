...
Executed via the Node.js CLI script. Run in Bash (`<skill_path>` should be replaced with the actual installed path of this tool). File paths are relative to the Gateway workspace directory (or absolute):

```bash
node <skill_path>/cli.mjs tasks/abc/report.pdf tasks/abc/chart.png --summary "Analysis Report" --task-id abc
# Or use JSON for per-file summary control:
node <skill_path>/cli.mjs --files-json '[{"path":"tasks/abc/report.pdf","summary":"Report"}]' --task-id abc
```
