...
- {file-name}: {行数:<N>;字节:<B>}
```
- 每个数据收集 sub-agent 回读原始数据做摘要，并落盘 output_file `sub-output-<short_title>.<ext>`。**摘要必须自足**：包含关键数据点、~100字概述、重要资讯列表（`{资讯分类:id} 标题`，按重要性降序）、行情/资金异动要点（如适用），800 tokens 以内
...
- 主 Agent 只读取 `sub-output-*.md` 摘要与 `final-output-*.md` 分析报告。
- 主 Agent 生成/整合终稿时，首次 `write` 写入标题，后续章节用 当前 Agent 支持的追加或编辑操作 逐节追加，禁止整篇覆盖重写；修改已写内容用 `edit`。
...
- 写入节奏：`write`（标题）→ 当前 Agent 支持的追加或编辑操作（逐章节），禁止整篇覆盖重写。
...
2. 立即 当前 Agent 支持的追加或编辑操作 写入 `## 二、 基本面剖析（价值之锚）` + 报告全文内容（原样搬入，禁止压缩，剥离尾注暂时存供 Stage 4 使用）。
3. 暂存基本面关键结论（基本面判断、估值区间、核心风险，各≤50字），供 Stage 3 使用。

**Step 2.2 情绪章节**
1. `read` 情绪分析师报告（完整读取）。
2. 立即 当前 Agent 支持的追加或编辑操作 写入 `## 三、 情绪与消息面（资金博弈）` + 报告全文内容（原样搬入，禁止压缩，剥离尾注暂时存供 Stage 4 使用）。
3. 暂存情绪关键结论（多空倾向、核心分歧点、资金态度，各≤50字），供 Stage 3 使用。

**Step 2.3 技术面章节**
1. `read` 技术面分析师报告（完整读取）。
2. 立即 当前 Agent 支持的追加或编辑操作 写入 `## 四、 技术面量化（量价结构）` + 报告全文内容（原样搬入，禁止压缩，剥离尾注暂时存供 Stage 4 使用）。
...
4. 当前 Agent 支持的追加或编辑操作 将 `## 一、 研报摘要` 章节**插入到标题行之后、第二章之前**（用 `edit` 在标题行后插入）。

### Stage 4：尾部章节与完整性检查
1. 将上面三份尾注合并成汇总尾注，当前 Agent 支持的追加或编辑操作 追加 `[参考资料]`（汇总三份报告的引用）、`[图表数据]`、`[AI生成提示]`。
