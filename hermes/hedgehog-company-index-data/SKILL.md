...
4. **统一调度**：使用 `node ${HERMES_SKILL_DIR}/scripts/call_api.js --api <接口名> --params '<JSON字符串>' --dir <sessionTaskDir>` 执行调用。
5. **输出策略（脚本自动决定）**：
...
**典型用例**：查询股票一个月收盘价 `node ${HERMES_SKILL_DIR}/scripts/call_api.js --api queryStockDaily --params '{"stock_code":"000001.SZ","start_date":"2023-05-01","end_date":"2023-05-31","fields":"trade_date,close"}' --dir <sessionTaskDir>`
