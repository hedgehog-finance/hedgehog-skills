...
function requireNonEmptyFile(filePath, label) {
  if (!existsSync(filePath) || statSync(filePath).size === 0) throw new Error(`${label} did not produce a non-empty render`);
}
...
function validateWithLibreOffice(filePath) {
  ...
  try {
    ...
    requireNonEmptyFile(join(workDir, basename(filePath).replace(/\.pptx$/i, ".pdf")), "LibreOffice");
...
function validateWithKeynote(filePath) {
  if (process.platform !== "darwin" || !existsSync("/Applications/Keynote.app")) throw new Error("Keynote open/render test requested, but Keynote is not installed on macOS");
...
function validateWithPowerPoint(filePath) {
  if (process.platform !== "darwin" || !existsSync("/Applications/Microsoft PowerPoint.app")) throw new Error("PowerPoint open/render test requested, but Microsoft PowerPoint is not installed on macOS");
