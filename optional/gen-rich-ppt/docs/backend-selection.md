...
```text
我检查后没有可用的内置图片生成工具，或内置工具缺少本页必需能力，因此准备使用本地 API/CLI fallback 生成样张；它会按顺序读取 HogAgent skill 配置、环境变量和 ~/.gen-rich-ppt/.env。可以开始生成 1 页样张吗？
```
