...
- User custom styles: `${GEN_RICH_PPT_HOME:-~/.gen-rich-ppt}/references/*.md`. They live outside the skill install so they survive skill updates and reinstalls.
...
This list only tracks built-in styles shipped with the skill. Do not add user custom styles here; they are saved to `${GEN_RICH_PPT_HOME:-~/.gen-rich-ppt}/references/` via `docs/style-library.md` and discovered by directory scan, so they need no registration.
