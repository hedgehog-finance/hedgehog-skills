...
3. **宏观穿透**：Sub-agent 调取新闻 queryNewsAnalysis（`start_date=[前一日]`, `importance_score=4`, `news_type='macro'`），聚焦市场热点、核心数据、央行政策及黑天鹅。提取信息（<500字），文末追加 `[资料来源]` 章节，列出 `{类型(新闻|研报|公告)/id}: 标题`。将结果保存到独立文件，返回完成状态和文件名。
...
聚焦变革、数据与并购。提取信息（<500字），文末追加 `[资料来源]` 章节，列出格式同上。将结果保存到独立文件，返回完成状态和文件名。
...
死盯业绩、重组、人事及监管函。单股提炼（<200字），文末追加 `[资料来源]` 章节，列出格式同上。将所有股票的提炼结果保存到一个独立文件，返回完成状态和文件名。
6. **数据阻断**：静待全量回传，读取所有中间输出文件，严禁脱离接口捏造数据。
