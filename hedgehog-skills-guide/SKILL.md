...
- `hedgehog-news-and-reports`: 文本情报库。获取宏观、行业、个股的快讯、新闻、研报与公告。自带量化过滤条件：`importance_score` 重要度(0~5)、`market_sentiment_score` 市场情绪(-5~5)、`stock_impact_score` 个股影响因子(-5~5)。
...
- `hedgehog-calculator`: 高精度计算器。执行数学表达式计算、复杂财务比率与金融公式运算。
